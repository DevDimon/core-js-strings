...
function isPalindrome(str) {
  const delChar = '!?, ';
  let str1 = str;
  for (let i = 0; i < delChar.length; i += 1) {
    str1 = str1.replaceAll(delChar[i], '');
  }
  const str2 = str1.toUpperCase();
  return str2.split('').reverse().join('') === str1.toUpperCase();
}
...
function getCardId(value) {
  ...
  return suits.indexOf(suit) * 13 + values.indexOf(rank);
}
